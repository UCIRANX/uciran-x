...
//   OPENMODEL_API_KEY    — کلید OpenModel (از console.openmodel.ai)
...
// ─── OpenModel ───────────────────────────────────────────────
const OPENMODEL_BASE  = 'https://api.openmodel.ai/v1/messages';
const OPENMODEL_MODEL = 'deepseek-v4-flash';
const OPENMODEL_ID    = 'openmodel';
const OPENMODEL_NAME  = 'DeepSeek V4 Flash';
...
const ENGINES = {
  ...
  openmodel: { label: '🟢 DeepSeek V4 Flash'   },
...
async function callAI(messages, imageContent, engine, isMedia, env, forceSearch = false) {
  ...
  // حالت دستی
  if (engine !== 'auto') {
    if (engine === OPENMODEL_ID) {
      if (isMedia) throw new Error('DeepSeek V4 Flash از تصویر پشتیبانی نمی‌کنه.');
      if (!env.OPENMODEL_API_KEY) throw new Error('OPENMODEL_API_KEY تنظیم نشده.');
      return callOpenModel(messages, env);
    }
    if (engine === GEMINI_ID) {
      if (!getGeminiKeys(env).length) throw new Error('GEMINI_API_KEYS تنظیم نشده.');
  ...
  for (const step of FALLBACK_ORDER) {
    try {
      if (step.type === 'openmodel') {
        if (isMedia || !env.OPENMODEL_API_KEY) continue;
        return await callOpenModel(messages, env);
      }
      ...
      const label = step.type === 'openmodel' ? OPENMODEL_NAME
        : step.type === 'geminiapi' ? GEMINI_NAME
...
// ─── OpenModel ───────────────────────────────────────────────
async function callOpenModel(messages, env) {
  ...
  const payload = { model: OPENMODEL_MODEL, max_tokens: 2048, messages: chatMsgs };
  if (systemMsg) payload.system = systemMsg.content;

  const res = await fetch(OPENMODEL_BASE, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-api-key': env.OPENMODEL_API_KEY, 'anthropic-version': '2023-06-01' },
    body: JSON.stringify(payload)
  });

  const bodyText = await res.text();
  if (!res.ok) throw new Error(`OpenModel ${res.status}: ${bodyText.slice(0, 150)}`);

  let data;
  try { data = JSON.parse(bodyText); } catch { throw new Error('OpenModel parse error'); }

  const text = (data?.content || []).filter(b => b.type === 'text').map(b => b.text).join('').trim();
  if (!text) throw new Error('OpenModel پاسخ خالی داد.');

  console.log(`✅ OpenModel: ${OPENMODEL_MODEL}`);
  return { reply: text, modelName: OPENMODEL_NAME };
}
...
async function handleCallback(cq, env) {
  ...
  if (!data.startsWith('engine:') || !chatId || !userId) return answerCallback(cq.id, env, '');
